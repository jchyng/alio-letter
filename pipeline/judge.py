...
- 트랙 이름을 보고 해당 트랙의 직무 유형이 사용자 희망 근무분야와 명백히 다르면 eligible=false, unmet에 "근무분야 불일치: 희망 분야와 다른 직무 유형" 추가
- unmet: 미충족 항목만 나열. 충족이면 빈 배열 []
- bonus_summary: 공고의 중복 적용 규칙을 반영한 최종 합계. 단위가 혼재하면 그대로 표기 (예: '10% + 3점')
- bonus_reasons: 각 항목은 아래 접두사 중 하나로 시작:
  - 접두사 없음: 사용자에게 현재 적용됨 (예: "취업지원대상자: 10%")
  - '확인 필요: ': 사용자 정보만으로 판단 불가한 항목 (예: "확인 필요: 관련 자격증 보유 여부")
  - '미적용: ': 현재 해당 없지만 노력으로 준비 가능한 항목 — 자격증·어학·경력·교육 등 (예: "미적용: 정보처리기사 보유 시 5점")
- 장애인·취업지원대상자·자립준비청년·저소득층·북한이탈주민·다문화가족자녀 등 법정 신분 기반 가산점은 해당 없으면 bonus_reasons에서 완전히 제외
- 중복 불가 항목은 적용 여부와 무관하게 나열 (각각 해당 접두사 붙임)
...
def _profile_summary(profile: UserProfile) -> str:
    ...
    if birth_year:
        age = datetime.date.today().year - birth_year  # 현재 연도 기준 만 나이 근사
...
def judge_track(
    ...
    bonus_points: str,
    client: genai.Client,
    preferred_work_fields: list[str] | None = None,
) -> TrackJudgment:
    """트랙 1개의 자격요건·가산점을 Gemini로 판정한다."""
    work_fields_text = (
        ", ".join(preferred_work_fields) if preferred_work_fields else "제한 없음"
    )
    prompt = PROMPT_TEMPLATE.format(
        profile_summary=_profile_summary(profile),
        track_name=track.get("track_name", ""),
        preferred_work_fields=work_fields_text,
